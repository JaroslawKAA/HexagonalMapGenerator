...
namespace HexagonTilemapEditor
{
    ...
    public class HunksManagerSo : SerializedScriptableObject
    {
        ...
        private HexagonTilemapEditor.HexagonTilemapEditorSO _mapConfig;
        ...
        [Button]
        private void UpdateHunks()
        {
            hunks.Clear();
            AssetDatabase
                .FindAssets($"t: {nameof(HunkSO)}")
                .ToList()
                .Select(AssetDatabase.GUIDToAssetPath)
                .Select(AssetDatabase.LoadAssetAtPath<HunkSO>)
                .ToList()
                .ForEach(hSo => AddHunk(hSo.MinMaxRange, AssetDatabase.GetAssetPath(hSo)));
            EditorUtility.SetDirty(this);
        }
        ...
        private int _minxModulo;
        private int _minYModulo;
